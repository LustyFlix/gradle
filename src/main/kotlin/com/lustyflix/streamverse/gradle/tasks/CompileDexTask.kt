                ...
                .use { classesInput ->
                    ...
                    for (file in files) {
                        ...
                        for (annotation in classNode.visibleAnnotations.orEmpty() + classNode.invisibleAnnotations.orEmpty()) {
                            if (annotation.desc == "Lcom/lustyflix/streamverse/plugins/StreamversePlugin;") {
                                val streamverse = project.extensions.getStreamverse()

                                require(streamverse.pluginClassName == null) {
                                    "Only 1 active plugin class per project is supported"
                                }

                                streamverse.pluginClassName = classNode.name.replace('/', '.')
